...
function loadConfig(name, properties) {
    GM_registerMenuCommand("打开设置中心(智能测试)", () => {
        ...
        findFastestSite(websites)
            .then(fastestSite => {
                if (fastestSite) {
                    ...
                    else window.location.href = fastestSite
                } else {
    ...
    if (location.href.match('yuhan-script-config.netlify.app') || location.href.match('user-script-config-form.vercel.app')|| location.href.match('yuhanawa.github.io/tools/userscriptconfig') || location.href.match('localhost')) {
