({ //屏蔽器总开关
    value: () => {
        const filter = () => {
            if (!get("block_on_home")  && (location.href.includes("twitter.com/home") || location.href.includes("x.com/home"))) return
            ...
            for (const article of articles) {
                try {
                    ...
                    if (blackList.has(id)) {
                        article.style.display = "none";
                        if (get("show_note") && blackList.get(id).notShowNote !== true && blackList.get(id).type !== "id" && blackList.get(id).type !== "id_sum") {
