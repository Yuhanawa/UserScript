({
  ...
  showInMenu: true,
    ...
    value: () => {
        ...
        var style = style + $STYLE(ing_in_iframe)
        var refreshHeight = () => {
                ...
                getPage(`/ajax/ing/GetIngList?IngListType=${IngListType}&PageIndex=${PageIndex}&PageSize=30&Tag=&_=${Date.now()}`, {
                    onload: (response) => {
                        try {
                            ...
                            document.querySelector('#feed_list').insertAdjacentHTML('beforeend', `<div class="feed_loading"><img align="absmiddle" src="//assets.cnblogs.com/images/loading.gif" alt=""> 正在加载数据...</div>`);
                            PageIndex++
                        } catch (e) {
